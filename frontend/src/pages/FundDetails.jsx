...
const calculatePerformanceMetrics = (navData) => {
  ...
  const sixMonthReturn =
    sixMonthData.length > 1
      ? ((sixMonthData[sixMonthData.length - 1].nav - sixMonthData[0].nav) / sixMonthData[0].nav) *
        100
      : null
  ...
  const threeMonthReturn =
    threeMonthData.length > 1
      ? ((threeMonthData[threeMonthData.length - 1].nav - threeMonthData[0].nav) /
          threeMonthData[0].nav) *
        100
      : null
    ...
    <Card className="bg-white/5 border-white/10 hover:border-white/20 transition-all duration-300 group overflow-hidden">
      <CardContent className="p-5">
        <HStack spacing={3} align="start">
          <div
            className={`w-10 h-10 rounded-lg bg-${color}-500/15 flex items-center justify-center group-hover:scale-105 transition-transform duration-300`}
          >
...
const FundDetail = () => {
  ...
  const formatChartData = () => {
    ...
    return [...fundData.data]
      .filter((item) => {
        const date = parseDate(getNavItemDateValue(item))
        return date && date <= todayEnd && typeof item.nav === 'number' && !isNaN(item.nav)
      })
      .sort((a, b) => {
        const dateA = parseDate(getNavItemDateValue(a))
        const dateB = parseDate(getNavItemDateValue(b))
    ...
    return (
      <Box minH="100vh" bg="#0a0c10">
        <Container maxW="5xl" py={20}>
          <ErrorAlert message={error} onRetry={fetchFundDetails} onDismiss={() => navigate('/')} />
              ...
              >
                <VStack align="flex-start" spacing={3} maxW="4xl">
                  ...
                  <motion.div variants={itemVariants}>
                    <h1 className="text-2xl md:text-4xl lg:text-5xl font-semibold tracking-tight leading-tight">
                      <AuroraText>
                        {getSchemeName().length > 80
                          ? getSchemeName().substring(0, 80) + '...'
                          : getSchemeName()}
                      </AuroraText>
                    </h1>
                  </motion.div>
                  ...
                  <motion.div variants={itemVariants}>
                    <Text color="gray-500" fontSize="sm" maxW="3xl" leading="relaxed">
                      Comprehensive NAV performance analytics with interactive visualization and
                ...
                <motion.div variants={itemVariants}>
                  <Button
                    asChild
                    variant="outline"
                    className="rounded-full border-white/15 hover:bg-white/10 text-sm"
                  >
                    <Link to="/">
                          ...
                          <div>
                            ...
                            <Text color="white" fontSize="sm" fontWeight="medium">
                              {performanceMetrics?.dataPoints?.toLocaleString() ||
                                fundData.data.length.toLocaleString()}{' '}
                              records
                            </Text>
              ...
              <CardContent className="p-5">
                ...
                <VStack align="flex-start" spacing={3}>
                  <Flex justify="space-between" w="full">
                    ...
                    <Badge
                      bg={
                        Math.abs(performanceMetrics?.volatility) > 2
                          ? 'rose-500/15'
                          : Math.abs(performanceMetrics?.volatility) > 1
                            ? 'amber-500/15'
                            : 'emerald-500/15'
                      }
                      color={
              ...
              <CardContent className="p-5">
                ...
                <VStack align="flex-start" spacing={3}>
                  <Text color="gray-400" fontSize="xs" leading="relaxed">
                    • Total return of{' '}
                    <span className={isPositiveReturn ? 'text-emerald-400' : 'text-rose-400'}>
                      {performanceMetrics?.totalReturn}%
                    </span>{' '}
                    over the tracked period.
                  </Text>
                  <Text color="gray-400" fontSize="xs" leading="relaxed">
                    • Daily volatility of {performanceMetrics?.volatility}% suggests{' '}
                    {Math.abs(performanceMetrics?.volatility) > 1.5
                      ? 'moderate to high'
                      : 'relatively stable'}{' '}
                    price movements.
                  </Text>
                  <Text color="gray-400" fontSize="xs" leading="relaxed">
                    •{' '}
                    {performanceMetrics?.oneYearReturn
                      ? `1-year return of ${performanceMetrics.oneYearReturn}%`
                      : 'Recent performance'}{' '}
          ...
          >
            <HStack spacing={2} align="center">
              ...
              <Text color="gray-600" fontSize="2xs" lineHeight="tall">
                NAV data sourced from MFAPI. Historical performance does not guarantee future
                results.
              </Text>
            </HStack>
            <Badge
