    ...
    >
      ...
      <div className="w-full rounded-xl overflow-hidden">
        <ResponsiveContainer width="100%" height={height}>
          <AreaChart
            data={filteredData}
            margin={{ top: 15, right: 20, left: 5, bottom: 15 }}
          >
            <defs>
            ...
            <Tooltip
                  ...
                  return (
                    <div className="bg-[#1a1f2e] border border-purple-500/30 rounded-lg p-3 shadow-xl backdrop-blur-sm">
                      ...
                      <p className="text-white text-base font-semibold">
                        NAV: {valuePrefix}{payload[0]?.value?.toFixed(3)}
                      </p>
                      <div className="mt-1.5 pt-1.5 border-t border-white/10">
                        <p className="text-purple-400 text-xs">
                          Value: {valuePrefix}{payload[0]?.value?.toFixed(2)}
                        </p>
                      </div>
                    </div>
                  );
              ...
              cursor={{ stroke: "rgba(167, 139, 250, 0.4)", strokeWidth: 1.5, strokeDasharray: "4 4" }}
            />
        ...
        <div className="flex flex-wrap gap-4 mt-4 pt-3 border-t border-white/5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-white text-xs font-mono">
              {valuePrefix}{filteredData[filteredData.length - 1]?.value?.toFixed(3)}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-white text-xs font-mono">
              {valuePrefix}{Math.max(...filteredData.map(d => d.value)).toFixed(3)}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-white text-xs font-mono">
              {valuePrefix}{Math.min(...filteredData.map(d => d.value)).toFixed(3)}
            </span>
          </div>
        </div>
      )}
